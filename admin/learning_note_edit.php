...
<head>
    ...
    <title><?php echo $id ? '編輯' : '新增'; ?>學習筆記 | Admin</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-slate-50 min-h-screen">
    <header class="bg-slate-900 text-white px-4 py-4"><div class="max-w-3xl mx-auto"><a href="learning_notes.php" class="text-slate-300 text-sm">← 返回</a></div></header>
    ...
    <script>const TOPICS=<?php echo json_encode($topicsJson, JSON_HEX_TAG|JSON_HEX_AMP); ?>;const EDIT_ID=<?php echo $id; ?>;</script>
    ...
    (async function(){
        ...
        if(EDIT_ID){
            const rows=await AdminApi.apiFetch('/admin/learning-notes');
        ...
        document.getElementById('edit-form').onsubmit=async(e)=>{
            ...
            try{await AdminApi.apiFetch('/admin/learning-notes',{method:'POST',body:payload});location.href='learning_notes.php';}catch(err){flash.textContent=err.message;flash.classList.remove('hidden');}
        };
        const delBtn=document.getElementById('btn-delete');
        if(delBtn) delBtn.onclick=async()=>{
            if(!confirm('確定刪除此學習筆記？')) return;
            try{await AdminApi.apiFetch('/admin/learning-notes',{method:'DELETE',body:{id:EDIT_ID}});location.href='learning_notes.php';}catch(err){flash.textContent=err.message;flash.classList.remove('hidden');}
        };
    })();
    </script>
</body>
