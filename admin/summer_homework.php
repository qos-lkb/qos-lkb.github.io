...
if ($canAny) {
    $list = $pdo->query(
        'SELECT id, slug, title_zh, title_en, form_level, content_type, status, pass_percent, updated_at
...
admin_page_start('暑期功課', 'summer_homework', [
    ...
    'wide' => true,
    'subtitle' => '中一／中二：閱讀或影片 + 選擇題／填充題；及格線預設 80%',
]);
?>
        <div class="bg-white rounded-xl border border-slate-200 overflow-x-auto shadow-sm">
            <table class="min-w-full text-sm">
                ...
                <tbody>
                <?php if ($list === []): ?>
                    <tr><td colspan="8" class="p-6 text-slate-500 text-center">尚未建立暑期功課。請按「新增習作」。</td></tr>
                <?php endif; ?>
