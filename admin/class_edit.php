...
bootstrap_public();
require_permission('class.manage_own', '../login.php?next=' . rawurlencode('admin/class_edit.php'));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'reset_invite' && $id > 0 && $row) {
        ...
        if ($res['ok']) {
            header('Location: class_edit.php?id=' . (int) $res['id']);
            exit;
        }
...
admin_page_start($id ? '編輯班級' : '新增班級', 'classes', [
    'actions' => admin_btn('classes.php', '返回列表', 'secondary'),
]);
?>
        <?php if ($error !== ''): ?>
        ...
        <form method="post" class="bg-white border border-slate-200 rounded-xl p-6 space-y-4 shadow-sm mb-8">
            ...
            <div>
                <label class="block text-sm font-medium text-slate-700">班級名稱</label>
            ...
            <?php if ($canAny): ?>
            <div>
                <label class="block text-sm font-medium text-slate-700">班級導師</label>
        ...
        <div class="bg-white border border-slate-200 rounded-xl p-6 shadow-sm mb-8">
            ...
            <form method="post" class="space-y-3">
                ...
                <button type="submit" class="bg-slate-700 text-white px-4 py-2 rounded-lg text-sm">加入班級</button>
            </form>
        </div>
