...
bootstrap_public();
require_permission('learning_tool.manage_own', '../login.php?next=' . rawurlencode('portal/learning_tool_edit.php'));
...
<head>
    ...
    <title><?php echo $id ? '編輯' : '新增'; ?>學習工具 | Portal</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-slate-50 min-h-screen">
    <header class="bg-slate-900 text-white px-4 py-4"><a href="learning_tools.php" class="text-slate-300 text-sm">← 返回</a></header>
    <main class="max-w-3xl mx-auto px-4 py-8">
        <p class="text-sm text-slate-600 mb-4">儲存為草稿或提交審核；發佈需管理員核准。</p>
        ...
        <form id="edit-form" class="space-y-4 bg-white rounded-xl border p-6">
            ...
            <div><div class="flex justify-between mb-2"><label class="text-sm font-medium">題目</label><button type="button" id="add-q" class="text-sm text-indigo-600">+ 新增</button></div><div id="questions"></div></div>
    ...
    (async function(){
        ...
        if(!EDIT_ID) AdminApi.renderQuestionBlock(AdminApi.blankQuestion(),0,qBox);
        else {
        ...
        document.getElementById('edit-form').onsubmit=async(e)=>{
            e.preventDefault();
            try{
                await AdminApi.apiFetch('/admin/learning-tools',{method:'POST',body:{
                    ...
                    description_zh:document.getElementById('desc-zh').value,status:document.getElementById('status').value,
