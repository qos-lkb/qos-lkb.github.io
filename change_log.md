...
- 課程暑期紀錄支援 `?cohort=previous`：以**本班名冊**顯示上學年級習作（中二→中一、中三→中二），方便新學年科任追收。
- API：`GET /admin/classes/{id}/summer-homework?cohort=previous`（CSV 相同）；回傳 `cohort_note`、`items_form_level`，學生可附 `prev_form_class`。
- 中二／中三課程在教師首頁與課程頁顯示「上學年追收」。
- 學生端年級改為「剛讀完的年級」：若已掛入本學年（例如 2026/27）選課，中二補交中一、中三補交中二；仍留在上學年課程者維持該年習作。6–8 月派發期用本班中一／中二年級。

## 2026-08-28

### 暑期功課紀錄：重新定義準時／遲交／未交／欠交
- **準時**：截止前首次及格；**遲交**：截止後首次及格。
- **未交**：截止前未及格；**欠交**：截止後未及格。
- 課程「暑期功課紀錄」矩陣篩選新增「欠交」；未完成清單含截止前／後未及格。
