...
const global = window;
    ...
    function progressBadge(progress, lang) {
        if (!progress || !progress.passed) {
            const status = progress && progress.submission_status;
            let statusLabel = st('未交', 'Not passed (before due)', lang);
            if (status === 'overdue') {
                statusLabel = st('欠交', 'Not passed (after due)', lang);
            }
            ...
            return `<span class="inline-flex flex-wrap gap-1 justify-end">
                <span class="text-xs px-2 py-0.5 rounded-full ${status === 'overdue' ? 'bg-red-100 text-red-900' : 'bg-slate-100 text-slate-700'} font-medium">${statusLabel}</span>
        ...
        let statusBadge = '';
        if (status === 'late') {
            statusBadge = `<span class="text-xs px-2 py-0.5 rounded-full bg-orange-100 text-orange-900">${st('遲交', 'Late pass', lang)}</span>`;
    ...
    function showResult(result, slug, lang, questions, opts) {
        ...
        box.innerHTML = `
            ...
            ${result.is_late && everPassed ? `<p class="mt-2 text-sm text-orange-800 font-medium">${st('首次及格時間在截止日期之後，狀態為「遲交」。', 'First pass was after the due date — status is “Late pass”.', lang)}</p>` : ''}
            ...
            ${passed
                ...
                : (everPassed
                    ...
                    : `<p class="mt-3 text-sm text-red-900 font-semibold" role="alert">${result.submission_status === 'overdue'
                        ? st('不及格：未達及格線，狀態為「欠交」。請重讀／重看內容後再次完成並提交。', 'Not passed: below the pass mark — status is “Not passed (after due)”. Review the content and complete the assessment again.', lang)
                        : st('不及格：未達及格線，狀態為「未交」。請重讀／重看內容後再次完成並提交。', 'Not passed: below the pass mark — status is “Not passed (before due)”. Review the content and complete the assessment again.', lang)}</p>`)}
