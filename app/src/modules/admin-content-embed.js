(function (global) {
    ...
    async function loadPickerData() {
        ...
        const fetchJson = async (path) => {
            try {
                return await global.ScienceApi.apiFetch(path);
            } catch {
                return [];
            }
        };
        const [catalog, videos, banks] = await Promise.all([
            global.ScienceApi.apiFetch('/catalog'),
    ...
    async function openBankQuestions(bankId, bankSlug, bankTitle) {
        const detail = await global.ScienceApi.apiFetch('/admin/question-banks/' + bankId);
