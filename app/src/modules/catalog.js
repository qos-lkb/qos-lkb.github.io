...
const global = window;
    ...
    function renderLearningToolsList() {
        const container = document.getElementById('card-container');
        const lang = getLang();
        document.getElementById('page-title').textContent = t('試題庫測驗', 'Question bank quizzes');
        if (!learningTools.length) {
            container.innerHTML = `<p class="text-slate-500">${t('尚無已發佈的試題庫。', 'No published question banks yet.')}</p>`;
            return;
        }
