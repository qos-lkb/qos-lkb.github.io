const a=window;function t(l,n){return a.AppRouter&&a.AppRouter.t?a.AppRouter.t(l,n):l}function e(l){return a.AppRouter&&a.AppRouter.escapeHtml?a.AppRouter.escapeHtml(l):String(l||"")}function u(l){return a.AppRouter&&a.AppRouter.spaHref?a.AppRouter.spaHref(l):String(l||"")}function C(){const l=document.getElementById("sidebar");l&&(l.style.display="none")}function L(l,n){const v=Object.entries(l||{});return'<option value="">'+e(t("請選擇","Select"))+"</option>"+v.map(([d,m])=>`<option value="${e(d)}"${String(d)===String(n||"")?" selected":""}>${e(m)}</option>`).join("")}function D(l){const n=String(l||"").toUpperCase();return n==="E"?t("英文 (E)","English (E)"):n==="C"?t("中文 (C)","Chinese (C)"):"—"}function R(){if(!a.ScienceApi.getUser())return a.AppRouter.navigate("/login"),!1;const l=a.ScienceApi.hasPermission("class.manage_any"),n=a.ScienceApi.hasPermission("class.manage_own");return l||n}async function I(){var d;C();const l=document.getElementById("page-title"),n=document.getElementById("card-container");if(l&&(l.textContent=t("課程管理","Courses")),!R()){a.ScienceApi.getUser()&&(n.innerHTML=`<p class="text-red-600">${e(t("沒有權限。","Forbidden."))}</p>`);return}n.innerHTML=`<p class="text-slate-500">${e(t("載入中…","Loading…"))}</p>`;try{let x=function(s,i){h&&(h.textContent=s,h.classList.remove("hidden","text-emerald-700","text-red-600"),h.classList.add(i?"text-red-600":"text-emerald-700"))};var v=x;const m=await a.ScienceApi.apiFetch("/admin/classes"),y=m.classes||[],w=m.form_level_options||{},S=m.course_subject_options||{},c=m.teacher_options||[],o=a.ScienceApi.hasPermission("class.manage_any"),$=a.ScienceApi.getUser(),j=(()=>{const s=new Date().getFullYear();return s+"-"+(s+1)})(),A=o&&c.length?`<label class="text-sm">${e(t("任教老師","Teacher"))}
                    <select name="teacher_user_id" class="mt-1 w-full rounded border border-slate-300 px-2 py-1.5 text-sm">
                        ${c.map(s=>`<option value="${Number(s.id)}"${Number(s.id)===Number($.id)?" selected":""}>${e(s.label||s.display_name||s.email)}</option>`).join("")}
                    </select>
                ...
                </div>`;const h=document.getElementById("admin-courses-flash");n.querySelectorAll("[data-spa-nav]").forEach(s=>{s.addEventListener("click",i=>{i.preventDefault(),a.AppRouter.navigate(s.getAttribute("data-spa-nav"))})}),(d=document.getElementById("admin-course-create"))==null||d.addEventListener("submit",async s=>{s.preventDefault();const i=new FormData(s.target);try{const p=await a.ScienceApi.apiFetch("/admin/classes",{method:"POST",body:{name:String(i.get("name")||""),school_year:String(i.get("school_year")||""),form_level:String(i.get("form_level")||""),course_subject:String(i.get("course_subject")||""),teacher_user_id:parseInt(String(i.get("teacher_user_id")||$.id),10)||$.id,is_active:i.get("is_active")==="on"||i.get("is_active")==="1"}}),f=p.class&&p.class.id?Number(p.class.id):0;if(f>0){a.AppRouter.navigate("/admin/courses/"+f);return}x(t("已新增課程。","Course created."),!1),await I()}catch(p){x(p.message||t("儲存失敗","Save failed"),!0)}}),n.querySelectorAll(".admin-course-delete").forEach(s=>{s.addEventListener("click",async()=>{const i=parseInt(s.getAttribute("data-id")||"0",10);if(!(!i||!confirm(t("確定刪除此課程？","Delete this course?"))))try{await a.ScienceApi.apiFetch("/admin/classes/"+i,{method:"DELETE",body:{}}),x(t("已刪除。","Deleted."),!1),await I()}catch(p){x(p.message||t("刪除失敗","Delete failed"),!0)}})})}catch(m){n.innerHTML=`<p class="text-red-600">${e(m.message||t("載入失敗","Load failed"))}</p>`}}async function k(l){var y,w,S;C();const n=parseInt(l,10)||0,v=document.getElementById("page-title"),d=document.getElementById("card-container");if(v&&(v.textContent=t("編輯課程","Edit course")),!R()){a.ScienceApi.getUser()&&(d.innerHTML=`<p class="text-red-600">${e(t("沒有權限。","Forbidden."))}</p>`);return}if(n<=0){a.AppRouter.navigate("/admin/courses");return}d.innerHTML=`<p class="text-slate-500">${e(t("載入中…","Loading…"))}</p>`;try{let g=function(r,b){f&&(f.textContent=r,f.classList.remove("hidden","text-emerald-700","text-red-600"),f.classList.add(b?"text-red-600":"text-emerald-700"))};var m=g;const c=await a.ScienceApi.apiFetch("/admin/classes/"+n),o=c.class;if(!o){d.innerHTML=`<p class="text-red-600">${e(t("找不到課程。","Course not found."))}</p>`;return}const $=c.form_level_options||{},j=c.course_subject_options||{},A=c.teacher_options||[],E=c.students||[],h=!!c.can_edit_students,x=a.ScienceApi.hasPermission("class.manage_any"),s=a.ScienceApi.getUser(),i=x&&A.length?`<label class="block text-sm font-medium text-slate-700">${e(t("任教老師","Teacher"))}
                ...
                ${h?"":`<p class="text-sm text-slate-500">${e(t("加入／移出學生與修改修讀語言僅限管理員操作。","Only admins can enroll/remove students or edit MOI."))}</p>`}`;const f=document.getElementById("admin-course-edit-flash");d.querySelectorAll("[data-spa-nav]").forEach(r=>{r.addEventListener("click",b=>{b.preventDefault(),a.AppRouter.navigate(r.getAttribute("data-spa-nav"))})}),(y=document.getElementById("admin-course-edit"))==null||y.addEventListener("submit",async r=>{var N;r.preventDefault();const b=r.target,_=new FormData(b);try{await a.ScienceApi.apiFetch("/admin/classes/"+n,{method:"PUT",body:{name:String(_.get("name")||"").trim(),school_year:String(_.get("school_year")||"").trim(),form_level:String(_.get("form_level")||""),course_subject:String(_.get("course_subject")||""),teacher_user_id:parseInt(String(_.get("teacher_user_id")||s.id),10)||s.id,is_active:!!((N=b.querySelector('input[name="is_active"]'))!=null&&N.checked)}}),g(t("已儲存。","Saved."),!1),await k(String(n))}catch(H){g(H.message||t("儲存失敗","Save failed"),!0)}}),(w=document.getElementById("course-reset-invite"))==null||w.addEventListener("click",async()=>{if(confirm(t("重設邀請碼？舊碼將失效。","Reset invite code? Old code will stop working.")))try{const r=await a.ScienceApi.apiFetch("/admin/classes/"+n+"/invite",{method:"POST",body:{}}),b=document.getElementById("course-invite-code");b&&r.invite_code&&(b.textContent=r.invite_code),g(t("新邀請碼：","New invite: ")+(r.invite_code||""),!1)}catch(r){g(r.message||t("重設失敗","Reset failed"),!0)}}),(S=document.getElementById("course-edit-delete"))==null||S.addEventListener("click",async()=>{if(confirm(t("確定刪除此課程？學生選課紀錄將一併移除。","Delete this course? Enrollments will be removed.")))try{await a.ScienceApi.apiFetch("/admin/classes/"+n,{method:"DELETE",body:{}}),a.AppRouter.navigate("/admin/courses")}catch(r){g(r.message||t("刪除失敗","Delete failed"),!0)}})}catch(c){d.innerHTML=`<p class="text-red-600">${e(c.message||t("載入失敗","Load failed"))}</p>`}}a.AppAdmin=Object.assign(a.AppAdmin||{},{renderAdminCourses:I,renderAdminCourseEdit:k});
