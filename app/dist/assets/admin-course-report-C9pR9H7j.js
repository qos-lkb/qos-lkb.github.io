const r=window;function t(d,a){return r.AppRouter&&r.AppRouter.t?r.AppRouter.t(d,a):d}function e(d){return r.AppRouter&&r.AppRouter.escapeHtml?r.AppRouter.escapeHtml(d):String(d||"")}function i(d){return r.AppRouter&&r.AppRouter.spaHref?r.AppRouter.spaHref(d):String(d||"")}function F(){const d=document.getElementById("sidebar");d&&(d.style.display="none")}function P(){return r.ScienceApi.getUser()?r.ScienceApi.hasPermission("class.manage_any")||r.ScienceApi.hasPermission("class.manage_own"):(r.AppRouter.navigate("/login"),!1)}function c(d,a,$){return`<div class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
            ...
            <p class="text-2xl font-bold ${$||"text-slate-900"}">${a}</p>
                    ...
                    <h2 class="font-bold text-amber-900 mb-2">${e(t("全班薄弱課題 TOP ","Weak topics TOP ")+_.length)}</h2>
                    <ul class="text-sm text-amber-800 space-y-1">
                        ${_.map(s=>`<li>${e(s.name_zh||"")} — ${e(t("平均","avg"))} ${Number(s.avg_mastery)}%（${Number(s.student_count)} ${e(t("人","students"))}）</li>`).join("")}
                    </ul>
                    ...
                    <td class="p-3 text-xs">${Number(L.passed||0)}/${Number(L.total||0)}</td>
                    <td class="p-3 text-xs">${s.last_active_at?e(s.last_active_at):"—"}</td>
                    <td class="p-3">${e(O)}</td>
                </tr>`}).join(""),S=w.slice().sort((s,n)=>{const l=Number(s.avg_mastery||0),o=Number(n.avg_mastery||0);return o!==l?o-l:Number(n.minutes_week||0)-Number(s.minutes_week||0)}).slice(0,3),h=w.slice().sort((s,n)=>Number(n.minutes_week||0)-Number(s.minutes_week||0))[0]||null,j=S.length?`<div class="bg-indigo-50 border border-indigo-200 rounded-xl p-4 mb-6">
                    ...
                    <div class="space-y-2">
                        ${S.map((s,n)=>{const l=s.display_name||"",o=Number(s.avg_mastery||0),x=Number(s.minutes_week||0);return`<div class="flex flex-wrap items-center justify-between gap-3 p-3 bg-white border border-indigo-100 rounded-lg">
                                <span class="text-sm font-medium text-slate-800">${n+1}. ${e(l)}</span>
                                <span class="text-xs text-slate-600">${o}% · ${x} ${e(t("分鐘","min"))}</span>
                            </div>`}).join("")}
                    </div>
                </div>`:"",b=p.achievements_summary||{},A=b.top_streaks||[],R=b.badge_unlock_counts||[],M=`
                <div class="bg-emerald-50 border border-emerald-200 rounded-xl p-4 mb-6">
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            ${A.length?`<ul class="text-sm space-y-1">${A.map((s,n)=>`<li>${n+1}. ${e(s.display_name||"")} — ${Number(s.current_streak_days||0)} ${e(t("天","days"))}</li>`).join("")}</ul>`:`<p class="text-sm text-slate-500">${e(t("尚無資料","No data"))}</p>`}
                        </div>
                        <div>
                            <p class="text-xs font-medium text-emerald-900 mb-2">${e(t("徽章解鎖次數","Badge unlocks"))}</p>
                            ${R.length?`<ul class="text-sm space-y-1">${R.slice(0,5).map(s=>`<li>${e(s.label_zh||s.badge_id||"")} × ${Number(s.count||0)}</li>`).join("")}</ul>`:`<p class="text-sm text-slate-500">${e(t("尚無徽章","No badges yet"))}</p>`}
                        </div>
                    </div>
                ...
                </div>`;const v=document.getElementById("admin-course-report-flash");u.querySelectorAll("[data-spa-nav]").forEach(s=>{s.addEventListener("click",n=>{n.preventDefault(),r.AppRouter.navigate(s.getAttribute("data-spa-nav"))})}),(k=document.getElementById("report-export-csv"))==null||k.addEventListener("click",async()=>{try{const s=await r.ScienceApi.apiFetch("/teacher/classes/"+a+"/report.csv",{method:"GET"});if(!(s instanceof Response))throw new Error(t("匯出回應格式錯誤","Unexpected export response"));const n=await s.blob(),l=URL.createObjectURL(n),o=document.createElement("a");o.href=l,o.download="class-"+a+"-report.csv",document.body.appendChild(o),o.click(),o.remove(),URL.revokeObjectURL(l),y(t("已開始下載 CSV。","CSV download started."),!1)}catch(s){y(s.message||t("匯出失敗","Export failed"),!0)}})}catch(N){u.innerHTML=`<p class="text-red-600">${e(N.message||t("載入失敗","Load failed"))}</p>`}}r.AppAdmin=Object.assign(r.AppAdmin||{},{renderAdminCourseReport:V});
