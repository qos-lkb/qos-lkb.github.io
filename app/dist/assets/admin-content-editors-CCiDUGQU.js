const s=window;function e(l,i){return s.AppRouter&&s.AppRouter.t?s.AppRouter.t(l,i):l}function t(l){return s.AppRouter&&s.AppRouter.escapeHtml?s.AppRouter.escapeHtml(l):String(l||"")}function B(l){return String(l||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;")}function O(l){return s.AppRouter&&s.AppRouter.spaHref?s.AppRouter.spaHref(l):String(l||"")}function S(){const l=document.getElementById("sidebar");l&&(l.style.display="none")}function h(l){l.querySelectorAll("[data-spa-nav]").forEach(i=>{i.addEventListener("click",n=>{n.preventDefault(),s.AppRouter.navigate(i.getAttribute("data-spa-nav"))})})}function A(l,i){return s.ScienceApi.getUser()?!(!s.ScienceApi.hasPermission(l)&&!(i&&s.ScienceApi.hasPermission(i))):(s.AppRouter.navigate("/login"),!1)}function z(l,i){let n=`<option value="draft">${t(e("草稿","Draft"))}</option>`;return n+=`<option value="pending_review">${t(e("待審核","Pending review"))}</option>`,i&&(n+=`<option value="published">${t(e("已發佈","Published"))}</option>`),n}async function j(){const l={},i=[];let n;try{n=await s.ScienceApi.apiFetch("/admin/subjects")}catch{n=await s.ScienceApi.apiFetch("/subjects")}return(Array.isArray(n)?n:[]).forEach(o=>{i.push(o),l[Number(o.id)]=o.topics||[]}),{subjects:i,topicsBySubject:l}}function M(l,i,n,o){l.innerHTML='<option value="">—</option>',(i[Number(n)]||[]).forEach(r=>{const u=document.createElement("option");u.value=String(r.id),u.textContent=r.name_zh||r.name_en||"#"+r.id,o&&Number(r.id)===Number(o)&&(u.selected=!0),l.appendChild(u)})}function k(l,i,n,o,r){l.addEventListener("change",()=>{M(i,n,l.value,"")}),o&&(l.value=String(o),M(i,n,o,r||""))}function q(){return{stem_zh:"",stem_en:"",explanation_zh:"",explanation_en:"",options:[{text_zh:"",text_en:"",is_correct:!0},{text_zh:"",text_en:"",is_correct:!1},{text_zh:"",text_en:"",is_correct:!1},{text_zh:"",text_en:"",is_correct:!1}]}}function R(l,i,n){const o=document.createElement("div");o.className="border rounded-xl p-4 mb-4 bg-slate-50",o.innerHTML=`
            ...
            <textarea class="expl-en w-full border rounded p-2 text-sm" rows="2">${B(l.explanation_en||"")}</textarea>`;const r=o.querySelector(".options");(l.options||q().options).forEach((u,a)=>{const d=document.createElement("div");d.className="flex gap-2 items-start flex-wrap",d.innerHTML=`
                ...
                <input class="opt-en flex-1 border rounded p-1 text-sm min-w-[120px]" placeholder="Option EN" value="${B(u.text_en)}">`,r.appendChild(d)}),o.querySelector(".remove-q").onclick=()=>o.remove(),n.appendChild(o)}function U(l){return Array.from(l.querySelectorAll(":scope > div")).map((i,n)=>{const o=Array.from(i.querySelectorAll(".correct")).findIndex(a=>a.checked),r=i.querySelectorAll(".options > div"),u=Array.from(r).map((a,d)=>({text_zh:a.querySelector(".opt-zh").value,text_en:a.querySelector(".opt-en").value,is_correct:d===o,sort_order:d}));return{sort_order:n,stem_zh:i.querySelector(".stem-zh").value,stem_en:i.querySelector(".stem-en").value,explanation_zh:i.querySelector(".expl-zh").value,explanation_en:i.querySelector(".expl-en").value,options:u}})}async function P(l,i){try{const n=await s.ScienceApi.apiFetch("/articles/"+encodeURIComponent(l)+"/answers"),o={};(n.answers||[]).forEach(r=>{o[r.question_id]=r.correct_option_index}),i.forEach(r=>{const u=o[r.id];u!==void 0&&r.options&&r.options.forEach((a,d)=>{a.is_correct=d===u})})}catch{}return i}function w(l,i){return`
            ...
            </form>`,h(n);const p=document.getElementById("subject-id"),_=document.getElementById("topic-id");k(p,_,r,a&&a.subject_id,a&&a.topic_id),a&&(document.getElementById("title-zh").value=a.title_zh||"",document.getElementById("title-en").value=a.title_en||"",document.getElementById("slug").value=a.slug||"",document.getElementById("source-url-zh").value=a.embed_url_zh||a.embed_url||"",document.getElementById("source-url-en").value=a.embed_url_en||"",document.getElementById("status").value=a.status||"draft",document.getElementById("duration").value=a.duration_minutes||""),document.getElementById("edit-form").onsubmit=async v=>{v.preventDefault();const g={id:parseInt(document.getElementById("item-id").value,10)||void 0,title_zh:document.getElementById("title-zh").value,title_en:document.getElementById("title-en").value,slug:document.getElementById("slug").value,source_url_zh:document.getElementById("source-url-zh").value,source_url_en:document.getElementById("source-url-en").value,subject_id:document.getElementById("subject-id").value||null,topic_id:document.getElementById("topic-id").value||null,duration_minutes:document.getElementById("duration").value||null,status:document.getElementById("status").value};try{await s.ScienceApi.apiFetch("/admin/learning-videos",{method:"POST",body:g}),s.AppRouter.navigate("/admin/learning-videos")}catch(m){x(m.message||e("儲存失敗","Save failed"))}};const $=document.getElementById("btn-delete");$&&($.onclick=async()=>{if(confirm(e("確定刪除此影片？","Delete this video?")))try{await s.ScienceApi.apiFetch("/admin/learning-videos",{method:"DELETE",body:{id:o}}),s.AppRouter.navigate("/admin/learning-videos")}catch(v){x(v.message||e("刪除失敗","Delete failed"))}})}async function Q(l){S();const i=document.getElementById("page-title"),n=document.getElementById("card-container"),o=l?Number(l):0;if(i&&(i.textContent=o?e("編輯文章","Edit article"):e("新增文章","New article")),!A("article.manage_any","article.manage_own")){s.ScienceApi.getUser()&&(n.innerHTML=`<p class="text-red-600">${t(e("沒有權限。","Forbidden."))}</p>`);return}n.innerHTML=`${w("/admin/articles",e("← 返回列表","← Back to list"))}<p class="text-slate-500">${t(e("載入中…","Loading…"))}</p>`,h(n);let r={},u=[],a=null,d=[];try{const g=await j();if(u=g.subjects,r=g.topicsBySubject,o){if(a=await L("/admin/articles",o),!a)throw new Error(e("找不到文章。","Article not found."));const m=await s.ScienceApi.apiFetch("/articles/"+encodeURIComponent(a.slug));d=await P(a.slug,m.questions||[])}}catch(g){n.innerHTML=`${w("/admin/articles",e("← 返回列表","← Back to list"))}<p class="text-red-600">${t(g.message)}</p>`,h(n);return}const p=u.map(g=>`<option value="${Number(g.id)}">${t(g.name_zh||g.name_en)}</option>`).join("");n.innerHTML=`
            ...
            </form>`,h(n);const _=document.getElementById("questions");document.getElementById("add-q").onclick=()=>R(q(),_.children.length,_),d.forEach((g,m)=>R(g,m,_));const $=document.getElementById("subject-id"),v=document.getElementById("topic-id");k($,v,r,a&&a.subject_id,a&&a.topic_id),a&&(document.getElementById("title-zh").value=a.title_zh||"",document.getElementById("title-en").value=a.title_en||"",document.getElementById("slug").value=a.slug||"",document.getElementById("body-zh").value=a.body_zh||"",document.getElementById("body-en").value=a.body_en||"",document.getElementById("status").value=a.status||"draft",document.getElementById("reading-time").value=a.reading_time_minutes||""),document.getElementById("edit-form").onsubmit=async g=>{g.preventDefault();const m={id:parseInt(document.getElementById("item-id").value,10)||void 0,title_zh:document.getElementById("title-zh").value,title_en:document.getElementById("title-en").value,slug:document.getElementById("slug").value,body_zh:document.getElementById("body-zh").value,body_en:document.getElementById("body-en").value,subject_id:document.getElementById("subject-id").value||null,topic_id:document.getElementById("topic-id").value||null,reading_time_minutes:document.getElementById("reading-time").value||null,status:document.getElementById("status").value,questions:U(_)};try{await s.ScienceApi.apiFetch("/admin/articles",{method:"POST",body:m}),s.AppRouter.navigate("/admin/articles")}catch(I){x(I.message||e("儲存失敗","Save failed"))}}}async function V(l){S();const i=document.getElementById("page-title"),n=document.getElementById("card-container"),o=l?Number(l):0,r=s.ScienceApi.hasPermission("learning_note.manage_any"),u=s.ScienceApi.hasPermission("question_bank.manage_any")||s.ScienceApi.hasPermission("question_bank.manage_own");if(i&&(i.textContent=o?e("編輯學習筆記","Edit note"):e("新增學習筆記","New note")),!A("learning_note.manage_any","learning_note.manage_own")){s.ScienceApi.getUser()&&(n.innerHTML=`<p class="text-red-600">${t(e("沒有權限。","Forbidden."))}</p>`);return}n.innerHTML=`${w("/admin/learning-notes",e("← 返回列表","← Back to list"))}<p class="text-slate-500">${t(e("載入中…","Loading…"))}</p>`,h(n);let a={},d=[],p=null;try{const m=await j();if(d=m.subjects,a=m.topicsBySubject,o&&(p=await L("/admin/learning-notes",o),!p))throw new Error(e("找不到筆記。","Note not found."))}catch(m){n.innerHTML=`${w("/admin/learning-notes",e("← 返回列表","← Back to list"))}<p class="text-red-600">${t(m.message)}</p>`,h(n);return}const _=d.map(m=>`<option value="${Number(m.id)}">${t(m.name_zh||m.name_en)}</option>`).join("");if(n.innerHTML=`
            ...
            </form>`,h(n),s.AdminContentEmbed){const m=u?["video","simulation","question"]:["video","simulation"];s.AdminContentEmbed.init(["body-zh","body-en"],{tabs:m})}const $=document.getElementById("subject-id"),v=document.getElementById("topic-id");k($,v,a,p&&p.subject_id,p&&p.topic_id),p&&(document.getElementById("title-zh").value=p.title_zh||"",document.getElementById("title-en").value=p.title_en||"",document.getElementById("slug").value=p.slug||"",document.getElementById("body-zh").value=p.body_zh||"",document.getElementById("body-en").value=p.body_en||"",document.getElementById("status").value=p.status||"draft",document.getElementById("reading-time").value=p.reading_time_minutes||"",document.getElementById("list-sort").value=p.list_sort_order||0),document.getElementById("edit-form").onsubmit=async m=>{m.preventDefault();const I={id:parseInt(document.getElementById("item-id").value,10)||void 0,title_zh:document.getElementById("title-zh").value,title_en:document.getElementById("title-en").value,slug:document.getElementById("slug").value,body_zh:document.getElementById("body-zh").value,body_en:document.getElementById("body-en").value,subject_id:document.getElementById("subject-id").value||null,topic_id:document.getElementById("topic-id").value||null,reading_time_minutes:document.getElementById("reading-time").value||null,list_sort_order:parseInt(document.getElementById("list-sort").value,10)||0,status:document.getElementById("status").value};try{await s.ScienceApi.apiFetch("/admin/learning-notes",{method:"POST",body:I}),s.AppRouter.navigate("/admin/learning-notes")}catch(b){x(b.message||e("儲存失敗","Save failed"))}};const g=document.getElementById("btn-delete");g&&(g.onclick=async()=>{if(confirm(e("確定刪除此學習筆記？","Delete this note?")))try{await s.ScienceApi.apiFetch("/admin/learning-notes",{method:"DELETE",body:{id:o}}),s.AppRouter.navigate("/admin/learning-notes")}catch(m){x(m.message||e("刪除失敗","Delete failed"))}})}async function G(l){S();const i=document.getElementById("page-title"),n=document.getElementById("card-container"),o=l?Number(l):0,r=s.ScienceApi.hasPermission("simulation.manage_any");if(i&&(i.textContent=o?e("編輯模擬","Edit simulation"):e("新增模擬","New simulation")),!A("simulation.manage_any","simulation.manage_own")){s.ScienceApi.getUser()&&(n.innerHTML=`<p class="text-red-600">${t(e("沒有權限。","Forbidden."))}</p>`);return}n.innerHTML=`${w("/admin/simulations",e("← 返回列表","← Back to list"))}<p class="text-slate-500">${t(e("載入中…","Loading…"))}</p>`,h(n);let u={},a=[],d=null,p=[];try{const c=await j();if(a=c.subjects,u=c.topicsBySubject,o&&(d=await s.ScienceApi.apiFetch("/admin/simulations?id="+o),!d||!d.id))throw new Error(e("找不到模擬。","Simulation not found."));if(r)try{const y=await s.ScienceApi.apiFetch("/admin/users");p=Array.isArray(y)?y:y.users||y.items||[]}catch{p=[]}}catch(c){n.innerHTML=`${w("/admin/simulations",e("← 返回列表","← Back to list"))}<p class="text-red-600">${t(c.message)}</p>`,h(n);return}const _=s.ScienceApi.getUser(),$=d?d.owner_user_id:_&&_.id,v=Array.isArray(d&&d.tags)?d.tags.join(", "):"",g=a.map(c=>`<option value="${Number(c.id)}">${t((c.name_zh||"")+" / "+(c.name_en||""))}</option>`).join(""),m=p.map(c=>`<option value="${Number(c.id)}" ${Number(c.id)===Number($)?"selected":""}>${t((c.email||"")+" — "+(c.display_name||""))}</option>`).join(""),I="allow-scripts allow-forms allow-popups allow-modals allow-downloads";n.innerHTML=`
