const p=window;function e(r,n){return p.AppRouter&&p.AppRouter.t?p.AppRouter.t(r,n):r}function t(r){return p.AppRouter&&p.AppRouter.escapeHtml?p.AppRouter.escapeHtml(r):String(r||"")}function H(r){return p.AppRouter&&p.AppRouter.spaHref?p.AppRouter.spaHref(r):r}function at(){const r=document.getElementById("sidebar");r&&(r.style.display="none")}function G(r,n){const c=String(r||"");return c.length>n?c.slice(0,n)+"…":c}function rt(r){return{mcq:e("選擇","MCQ"),fill_blank:e("填充","Fill"),true_false:e("是非","T/F"),short_answer:e("短答","Short"),long_answer:e("長答","Long")}[r]||r||"—"}function L(r){return r==null?"—":r?e("是","Yes"):e("否","No")}function J(r){return r==null?"text-slate-400":r>=50?"text-red-700 font-semibold":r>=30?"text-orange-700":"text-emerald-700"}function lt(r,n,c){const f=location.pathname.split("/app")[0]+"/app",B="/admin/summer-homework/"+r+"/analytics",h=new URLSearchParams;n>0&&h.set("user_id",String(n)),c>0&&h.set("attempt_id",String(c));const A=h.toString();history.replaceState({path:B},"",f+B+(A?"?"+A:""))}function nt(r,n){return Array.isArray(r)&&r.find(c=>R(c)&&Number(c.question_id)===n)||null}function R(r){return r&&typeof r=="object"}function ot(r,n){let c=r.teacher_marks;if(!c&&r.teacher_marks_json)try{c=typeof r.teacher_marks_json=="string"?JSON.parse(r.teacher_marks_json):r.teacher_marks_json}catch{c=null}return R(c)&&(c[String(n)]||c[n])||null}async function Y(r){var U;at();const n=parseInt(r,10)||0,c=document.getElementById("page-title"),f=document.getElementById("card-container");if(c&&(c.textContent=e("呈交分析","Submission analytics")),!p.ScienceApi.getUser()){p.AppRouter.navigate("/login");return}if(n<=0){p.AppRouter.navigate("/admin/summer-homework");return}const B=new URLSearchParams(location.search);let h=parseInt(B.get("user_id")||"0",10)||0,A=parseInt(B.get("attempt_id")||"0",10)||0;f.innerHTML=`<p class="text-slate-500">${t(e("載入中…","Loading…"))}</p>`;try{const w=await p.ScienceApi.apiFetch("/admin/summer-homework/"+n+"/analytics"),P=w.item||{},N=w.analytics||{},D=w.students||[],z=w.questions||[],K=!!w.can_manage,Q={};z.forEach(s=>{Q[Number(s.id)]=s});let T=[];h>0&&(T=(await p.ScienceApi.apiFetch("/admin/summer-homework/"+n+"/attempts?user_id="+h)).attempts||[]);let i=null;A>0&&(i=T.find(s=>Number(s.id)===A)||null,!i&&h<=0&&(i=((await p.ScienceApi.apiFetch("/admin/summer-homework/"+n+"/attempts")).attempts||[]).find(m=>Number(m.id)===A)||null));const X=(D.find(s=>Number(s.user_id)===h)||{}).display_name||"",Z=P.title_zh||P.title_en||"#"+n,q=String(P.form_level)==="2"?e("中二","S2"):e("中一","S1"),tt=(N.questions||[]).map((s,m)=>{const o=Number(s.question_id),$=Q[o],d=$&&($.stem_zh||$.stem_en)||"",_=s.miss_rate_percent;let b="";if(s.type==="mcq"&&Array.isArray(s.options)&&s.options.length&&(b=`<tr class="border-t border-slate-50 bg-slate-50/40"><td colspan="7" class="p-0">
                        ...
                        ${s.options.map(a=>{const l=!!a.is_correct,M=a.text_zh||a.text_en||""||"—",j=a.select_rate_percent,E=a.wrong_select_rate_percent;return`<tr class="border-t border-slate-100/80 ${l?"text-emerald-800":Number(j||0)>=20?"text-red-800":"text-slate-700"}">
                                ...
                                <td class="px-3 py-2">${l||E==null?"—":t(String(E))+"%"}</td>
                            </tr>`}).join("")}
                ...
                </tr>${b}`}).join(""),et=D.map(s=>{const m=Number(s.user_id),o=h===m,$=!s.passed;return`<tr class="border-t border-slate-100 ${o?"bg-indigo-50/50":$?"bg-amber-50/70":""}">
                    ...
                    <td class="p-3 ${$?"text-amber-900 font-semibold":"text-emerald-800"}">${t(L(!!s.passed))}</td>
                    ...
                    <td class="p-3"><button type="button" class="sh-filter-user text-indigo-600 hover:underline" data-user-id="${m}">${t(e("詳細","Detail"))}</button></td>
                </tr>`}).join("");let W="";if(h>0){const s=T.length,m=T.map((o,$)=>{const d=s-$;return`<tr class="border-t border-slate-100 ${i&&Number(i.id)===Number(o.id)?"bg-amber-50":""}">
                        <td class="p-3">${d}</td>
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody>${m||`<tr><td colspan="6" class="p-6 text-slate-500 text-center">${t(e("此學生尚無呈交。","No attempts."))}</td></tr>`}</tbody>
                    </table>
                </div>`}let V="";if(i){const s=R(i.grading)?i.grading:null,m=R(s)&&Array.isArray(s.details)?s.details:[],o=R(i.responses)?i.responses:{},$=z.map((d,_)=>{const b=Number(d.id),a=nt(m,b),l=o[String(b)]??o[b]??null,M=d.stem_zh||d.stem_en||"",j=a?!!a.correct:null,E=j===null?`<span class="text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">${t(e("無評分","Ungraded"))}</span>`:j?`<span class="text-xs px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800">${t(e("正確","Correct"))}</span>`:`<span class="text-xs px-2 py-0.5 rounded-full bg-red-100 text-red-800">${t(e("錯誤","Wrong"))}</span>`;let v="";const I=String(d.question_type||"");if(I==="mcq"){let u=null;a&&Object.prototype.hasOwnProperty.call(a,"selected_option_index")?u=a.selected_option_index!=null?Number(a.selected_option_index):null:l&&l.selected_option_index!=null&&(u=Number(l.selected_option_index));const x=a&&a.correct_option_index!=null?Number(a.correct_option_index):null;v='<ul class="text-sm space-y-1">'+(d.options||[]).map((S,g)=>{const O=String.fromCharCode(65+g),k=S.text_zh||S.text_en||"",y=[];u!==null&&g===u&&y.push(e("學生選","Chosen")),x!==null&&g===x&&y.push(e("正確答案","Answer"));let C="";return x!==null&&g===x?C="text-emerald-800":u!==null&&g===u&&(C="text-red-800"),`<li class="${C}"><span class="font-bold text-indigo-600 mr-1">${O}</span>${t(k)}${y.length?` <span class="text-xs text-slate-500">（${t(y.join(" · "))}）</span>`:""}</li>`}).join("")+"</ul>"}else if(I==="true_false"){const u=a&&Object.prototype.hasOwnProperty.call(a,"selected_bool")?a.selected_bool:l&&Object.prototype.hasOwnProperty.call(l,"selected_bool")?l.selected_bool:null,x=a&&Object.prototype.hasOwnProperty.call(a,"correct_bool")?a.correct_bool:d.correct_bool??null;v=`<p class="text-sm">${t(e("學生","Student"))}：${t(L(u))} · ${t(e("正解","Answer"))}：${t(L(x))}</p>`}else if(I==="short_answer"){const u=a&&a.given!=null?String(a.given):l?String(l.text||""):"";v=`<p class="text-sm font-mono">${t(u!==""?u:e("（空白）","(blank)"))}</p>`}else if(I==="long_answer"){const u=a&&a.given!=null?String(a.given):l?String(l.text||""):"",x=ot(i,b)||{},S=d.max_score!=null?d.max_score:5;v=`<div class="text-sm whitespace-pre-wrap bg-slate-50 border rounded-lg p-3 mb-3">${t(u!==""?u:e("（空白）","(blank)"))}</div>
                            ...
                            </form>`}else{const u=a&&Array.isArray(a.blanks)?a.blanks:[],x=l&&Array.isArray(l.blanks)||l&&R(l.blanks)?l.blanks:{};v=(d.blanks||[]).map((S,g)=>{const O=Number(S.blank_index??g+1),k=u.find(st=>Number(st.blank_index)===O)||null;let y=k&&k.given!=null?String(k.given):"";y||(y=x[g]!=null?String(x[g]):x[String(g)]!=null?String(x[String(g)]):"");const C=k?!!k.correct:null;return`<div class="text-sm mb-2">
                                <span class="text-slate-500">${t(e("空格","Blank"))} ${O}：</span>
                    ...
                    ${!m.length&&!Object.keys(o).length?`<p class="text-slate-500 text-sm">${t(e("此筆呈交沒有可顯示的作答／評分明細。","No response/grading detail for this attempt."))}</p>`:`<div class="space-y-4">${$}</div>`}
                </div>`}f.innerHTML=`
                <div class="mb-4 flex flex-wrap gap-3 items-center">
                    <a href="${t(H("/admin/summer-homework"))}" data-spa-nav="/admin/summer-homework" class="text-sm text-indigo-700 hover:underline">${t(e("← 習作列表","← Homework list"))}</a>
                    <a href="${t(H("/admin/summer-homework/"+n+"/view"))}" data-spa-nav="/admin/summer-homework/${n}/view" class="text-sm text-slate-600 hover:underline">${t(e("內容／答案","Content / answers"))}</a>
                    ${K?`<a href="${t(H("/admin/summer-homework/"+n+"/edit"))}" data-spa-nav="/admin/summer-homework/${n}/edit" class="text-sm text-slate-600 hover:underline">${t(e("編輯習作","Edit item"))}</a>`:""}
                </div>
                ...
                ${W}
                ${V}`;async function F(s,m){var o;lt(n,s||0,m||0),await Y(String(n)),m>0&&((o=document.getElementById("attempt-detail"))==null||o.scrollIntoView({behavior:"smooth",block:"start"}))}f.querySelectorAll(".sh-filter-user").forEach(s=>{s.addEventListener("click",()=>F(parseInt(s.getAttribute("data-user-id")||"0",10),0))}),(U=document.getElementById("sh-clear-user"))==null||U.addEventListener("click",()=>F(0,0)),f.querySelectorAll(".sh-open-attempt").forEach(s=>{s.addEventListener("click",()=>F(h,parseInt(s.getAttribute("data-attempt-id")||"0",10)))}),f.querySelectorAll(".sh-mark-form").forEach(s=>{s.addEventListener("submit",async m=>{m.preventDefault();const o=s.getAttribute("data-attempt"),$=s.getAttribute("data-qid"),d=s.querySelector(".mark-flash"),_={};_[$]={score:parseFloat(s.querySelector(".mark-score").value||"0"),comment:s.querySelector(".mark-comment").value||""};try{await p.ScienceApi.apiFetch("/admin/summer-homework/attempts/"+o+"/marks",{method:"POST",body:{marks:_}}),d&&(d.textContent=e("已儲存","Saved"),d.className="mark-flash text-xs text-emerald-700")}catch(b){d&&(d.textContent=b.message||e("失敗","Failed"),d.className="mark-flash text-xs text-red-600")}})})}catch(w){f.innerHTML=`<p class="text-red-600">${t(w.message||e("載入失敗","Load failed"))}</p>`}}p.AppAdmin=Object.assign(p.AppAdmin||{},{renderAdminSummerAnalytics:Y});
