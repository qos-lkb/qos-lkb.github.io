const n=window;function t(s,a){return n.AppRouter&&n.AppRouter.t?n.AppRouter.t(s,a):s}function e(s){return n.AppRouter&&n.AppRouter.escapeHtml?n.AppRouter.escapeHtml(s):String(s||"")}function _(s){return n.AppRouter&&n.AppRouter.spaHref?n.AppRouter.spaHref(s):String(s||"")}function T(){return(n.ScienceApi&&typeof n.ScienceApi.SITE_BASE=="string"?n.ScienceApi.SITE_BASE:typeof n.__SITE_BASE__=="string"?n.__SITE_BASE__:"")+"/codespace/index.html"}function A(){const s=document.getElementById("sidebar");s&&(s.style.display="none")}function E(){return n.ScienceApi.getUser()?!!n.ScienceApi.hasPermission("user.manage"):(n.AppRouter.navigate("/login"),!1)}function f(s){s.querySelectorAll("[data-spa-nav]").forEach(a=>{a.addEventListener("click",r=>{r.preventDefault(),n.AppRouter.navigate(a.getAttribute("data-spa-nav"))})})}function m(s,a,r){s&&(s.textContent=a,s.classList.remove("hidden","text-emerald-700","text-red-600","bg-emerald-50","bg-red-50","border-emerald-200","border-red-200"),s.classList.add(r?"text-red-600":"text-emerald-700"),s.classList.contains("border")&&s.classList.add(r?"bg-red-50":"bg-emerald-50",r?"border-red-200":"border-emerald-200"))}function H(s){const a=s.display_name||s.name_zh||s.name_en||"",r=s.email||"";return r?`${a} (${r})`:a||"#"+s.id}function M(s){let a=s.yearText||(s.yearFrom||"")+"-"+(s.yearEnd||"");return s.thisYear&&(a+=t("（本學年）"," (current)")),`${a} [${s.yearId}]`}function Q(s){const a=(s.kla_name_zh||"").trim(),r=(s.kla_name_en||"").trim(),o=(s.kla_code||"").trim();let d=a||r||o||"#"+s.kla_id;return o&&d!==o&&(d+=" ["+o+"]"),d}function x(){return`
            ...
            </div>`,f(a);const r=document.getElementById("db-export-btn"),o=document.getElementById("db-export-flash");r.addEventListener("click",async()=>{r.disabled=!0,m(o,t("正在匯出，請稍候…","Exporting…"),!1),o.classList.remove("hidden");try{const d=await n.ScienceApi.apiFetch("/admin/db/export",{method:"POST",body:{},rawResponse:!0}),p=await R(d);m(o,t("已開始下載 ","Download started: ")+p,!1)}catch(d){m(o,d.message||t("匯出失敗","Export failed"),!0)}finally{r.disabled=!1}})}async function C(){A();const s=document.getElementById("page-title"),a=document.getElementById("card-container");if(s&&(s.textContent=t("匯入資料庫","Import database")),!E()){n.ScienceApi.getUser()&&(a.innerHTML=`<p class="text-red-600">${e(t("沒有權限。","Forbidden."))}</p>`);return}a.innerHTML=`${x()}<p class="text-slate-500">${e(t("載入中…","Loading…"))}</p>`,f(a);let r;try{r=await n.ScienceApi.apiFetch("/admin/db/import-status")}catch(S){a.innerHTML=`${x()}<p class="text-red-600">${e(S.message||t("載入失敗","Load failed"))}</p>`,f(a);return}const o=!!r.wipe_allowed,d=r.confirm_phrase||"DELETE ALL TABLES",p=r.app_env||"",l=r.schema_name||"";a.innerHTML=`
            ${x()}
            ...
            </form>`,f(a);const g=document.getElementById("db-import-form"),y=document.getElementById("db-import-flash");g.addEventListener("submit",async S=>{if(S.preventDefault(),!o)return;const L=document.getElementById("sql_file"),k=document.getElementById("confirm_wipe").checked,q=String(document.getElementById("confirm_phrase").value||"").trim();if(!L.files||!L.files[0]){m(y,t("請選擇 SQL 檔案。","Choose an SQL file."),!0),y.classList.remove("hidden");return}const u=new FormData;u.append("sql_file",L.files[0]),k&&u.append("confirm_wipe","1"),u.append("confirm_phrase",q);const h=g.querySelector('button[type="submit"]');h.disabled=!0,m(y,t("正在匯入，請稍候…","Importing…"),!1),y.classList.remove("hidden");try{const b=await n.ScienceApi.apiFetch("/admin/db/import",{method:"POST",body:u}),i=b.tables!=null?b.tables:"?",v=b.dropped!=null?b.dropped:"?";m(y,t(`匯入完成：刪除 ${v} 張表，現有 ${i} 張表。`,`Import done: dropped ${v}, now ${i} tables.`),!1)}catch(b){m(y,b.message||t("匯入失敗","Import failed"),!0)}finally{h.disabled=!1}})}async function B(){A();const s=document.getElementById("page-title"),a=document.getElementById("card-container");if(s&&(s.textContent=t("資料字典","Data dictionary")),!E()){n.ScienceApi.getUser()&&(a.innerHTML=`<p class="text-red-600">${e(t("沒有權限。","Forbidden."))}</p>`);return}a.innerHTML=`${x()}<p class="text-slate-500">${e(t("載入中…","Loading…"))}</p>`,f(a);let r;try{r=await n.ScienceApi.apiFetch("/admin/data-dictionary")}catch(p){a.innerHTML=`${x()}<p class="text-red-600">${e(p.message||t("載入失敗","Load failed"))}</p>`,f(a);return}const o=!!r.exists;let d="";o&&n.AppMarkdown&&n.AppMarkdown.renderMarkdownToHtml?d=n.AppMarkdown.renderMarkdownToHtml(r.markdown||""):o&&(d=`<pre class="whitespace-pre-wrap text-xs">${e(r.markdown||"")}</pre>`),a.innerHTML=`
            ${x()}
            ...
            ${o?`<article id="dd-body" class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 sm:p-8 overflow-x-auto prose prose-slate max-w-none">${d}</article>`:`<div class="bg-amber-50 border border-amber-200 text-amber-900 rounded-xl p-6 text-sm">${e(t("尚未找到 data_dictionary.md。請按「重新產生」。","data_dictionary.md missing. Click Regenerate."))}</div>`}`,f(a),document.getElementById("dd-regenerate-btn").addEventListener("click",async()=>{const p=document.getElementById("dd-regenerate-btn"),l=document.getElementById("dd-flash");p.disabled=!0,m(l,t("產生中…","Generating…"),!1),l.classList.remove("hidden");try{const g=await n.ScienceApi.apiFetch("/admin/data-dictionary/regenerate",{method:"POST",body:{}});m(l,t(`已更新（${g.table_count||0} 張資料表）。重新載入中…`,`Updated (${g.table_count||0} tables). Reloading…`),!1),await B()}catch(g){m(l,g.message||t("產生失敗","Regenerate failed"),!0),p.disabled=!1}})}async function N(){A();const s=document.getElementById("page-title"),a=document.getElementById("card-container");if(s&&(s.textContent=t("QSIS 匯入","QSIS import")),!E()){n.ScienceApi.getUser()&&(a.innerHTML=`<p class="text-red-600">${e(t("沒有權限。","Forbidden."))}</p>`);return}const r=n.ScienceApi.getUser();a.innerHTML=`${x()}<p class="text-slate-500">${e(t("載入中…","Loading…"))}</p>`,f(a);let o;try{o=await n.ScienceApi.apiFetch("/admin/qsis/status")}catch(i){a.innerHTML=`${x()}<p class="text-red-600">${e(i.message||t("載入失敗","Load failed"))}</p>`,f(a);return}const d=o.connection||{},p=!!(o.configured&&d.ok),l=o.years||[],g=o.klas||[],y=o.teachers||[],S=o.current_year_id||l[0]&&l[0].yearId||"",L=l.map(i=>`<option value="${e(i.yearId)}" ${i.yearId===S?"selected":""}>${e(M(i))}</option>`).join(""),k=[`<option value="0">${e(t("全部 KLA","All KLAs"))}</option>`].concat(g.map(i=>`<option value="${Number(i.kla_id)}">${e(Q(i))}</option>`)).join(""),q=y.map(i=>`<option value="${Number(i.id)}" ${r&&Number(i.id)===Number(r.id)?"selected":""}>${e(H(i))}</option>`).join("");if(a.innerHTML=`
            ${x()}
            ...
            </div>`,f(a),!p)return;const u=document.getElementById("qsis-flash"),h=document.getElementById("qsis-courses");async function b(){const i=document.getElementById("qsis-year").value,v=document.getElementById("qsis-kla").value;h.innerHTML=`<p class="p-3 text-slate-500 text-sm">${e(t("載入課程…","Loading courses…"))}</p>`;try{const I=(await n.ScienceApi.apiFetch(`/admin/qsis/courses?year_id=${encodeURIComponent(i)}&kla_id=${encodeURIComponent(v)}`)).courses||[];if(!I.length){h.innerHTML=`<p class="p-3 text-slate-500 text-sm">${e(t("此條件下沒有課程。","No courses for this filter."))}</p>`;return}h.innerHTML=`<ul class="divide-y divide-slate-100">${I.map($=>`<li class="px-3 py-2 text-sm flex items-center gap-2">
