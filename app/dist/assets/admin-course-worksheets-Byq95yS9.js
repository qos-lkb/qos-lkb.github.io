const i=window;function e(n,r){return i.AppRouter&&i.AppRouter.t?i.AppRouter.t(n,r):n}function t(n){return i.AppRouter&&i.AppRouter.escapeHtml?i.AppRouter.escapeHtml(n):String(n||"")}function y(n){return i.AppRouter&&i.AppRouter.spaHref?i.AppRouter.spaHref(n):String(n||"")}function P(){const n=document.getElementById("sidebar");n&&(n.style.display="none")}function k(n){return{draft:e("草稿","Draft"),active:e("進行中","Active"),closed:e("已結束","Closed"),pending:e("未開始","Pending"),submitted:e("已提交","Submitted"),graded:e("已評分","Graded"),pending_review:e("待審","Pending review"),published:e("已發佈","Published")}[n]||n}function N(n){if(!n)return"—";if(n.question_type==="mcq"||n.question_type==="true_false"){const r=["A","B","C","D"],d=n.selected_option_index;return n.question_type==="true_false"?d===0?e("是","Yes"):d===1?e("否","No"):"—":r[d]!=null?r[d]:String(d+1)}return n.question_type==="short_answer"?n.response_text||e("（空白）","(blank)"):n.question_type==="long_answer"&&n.parts?n.parts.map(r=>"("+String.fromCharCode(97+r.part_index)+") "+r.text).join(" / ")||e("（空白）","(blank)"):n.question_type==="fill_blank"&&n.blanks?n.blanks.map(r=>"["+(r.blank_index+1)+"] "+r.text).join(" ")||e("（空白）","(blank)"):"—"}function M(){return i.ScienceApi.getUser()?i.ScienceApi.hasPermission("worksheet.assign_own")||i.ScienceApi.hasPermission("class.manage_any")||i.ScienceApi.hasPermission("worksheet.manage_any")||i.ScienceApi.hasPermission("worksheet.manage_own"):(i.AppRouter.navigate("/login"),!1)}async function F(n){var S,E,I;P();const r=parseInt(n,10)||0,d=document.getElementById("page-title"),h=document.getElementById("card-container");if(d&&(d.textContent=e("工作紙派發","Worksheet assignments")),!M()){i.ScienceApi.getUser()&&(h.innerHTML=`<p class="text-red-600">${t(e("沒有權限。","Forbidden."))}</p>`);return}if(r<=0){i.AppRouter.navigate("/admin/courses");return}const A=i.ScienceApi.hasPermission("worksheet.manage_any")||i.ScienceApi.hasPermission("worksheet.manage_own");h.innerHTML=`<p class="text-slate-500">${t(e("載入中…","Loading…"))}</p>`;try{let m=function(l,o){b&&(b.textContent=l,b.classList.remove("hidden"),b.className="text-sm mb-4 "+(o?"text-red-600":"text-emerald-700"))},f=function(){const l=document.getElementById("student-pick-wrap"),o=document.getElementById("assign-all").checked;l.classList.toggle("hidden",o),!o&&(l.innerHTML=H.map(s=>`<label class="flex items-center gap-2"><input type="checkbox" class="stu-pick" value="${Number(s.id)}"> ${t(s.name_zh||s.display_name||s.email)}</label>`).join(""))};var j=m,D=f;const T=(await i.ScienceApi.apiFetch("/admin/classes/"+r)).class||{};h.innerHTML=`
                <div class="mb-4 flex flex-wrap gap-3 items-center">
                    <a href="${t(y(`/admin/courses/${r}`))}" data-spa-nav="/admin/courses/${r}" class="text-sm text-indigo-700 hover:underline">${t(e("← 編輯課程","← Edit course"))}</a>
                    <a href="${t(y(`/admin/courses/${r}/report`))}" data-spa-nav="/admin/courses/${r}/report" class="text-sm text-slate-600 hover:underline">${t(e("學習報告","Report"))}</a>
                    <a href="${t(y(`/admin/courses/${r}/summer`))}" data-spa-nav="/admin/courses/${r}/summer" class="text-sm text-slate-600 hover:underline">${t(e("暑期功課","Summer HW"))}</a>
                ...
                <h2 class="text-lg font-bold text-slate-800 mb-2">${t(T.name||"")}</h2>
                ...
                <div class="grid lg:grid-cols-5 gap-6">
                ...
                </div>`;const b=document.getElementById("admin-ws-flash"),v=document.getElementById("assign-list"),B=document.getElementById("grade-panel"),z=document.getElementById("grade-title"),w=document.getElementById("grade-table-wrap");let H=[];async function C(){const l=await i.ScienceApi.apiFetch("/teacher/worksheets"),o=document.getElementById("worksheet-id");if(!l||!l.length){o.innerHTML=`<option value="">${t(e("— 請先設計工作紙 —","— Create a worksheet first —"))}</option>`;return}o.innerHTML=l.map(s=>{const u=s.is_mine?e("（我的）"," (mine)"):"",a=s.status==="published"?"":" ["+k(s.status)+"]";return`<option value="${Number(s.id)}">${t((s.title_zh||s.title_en||"")+u+a)}</option>`}).join("")}async function $(){const l=await i.ScienceApi.apiFetch("/teacher/classes/"+r+"/worksheet-assignments");H=l.students||[];const o=l.assignments||[];if(f(),!o.length){v.innerHTML=`<p class="text-slate-500">${t(e("尚無派發紀錄。","No assignments yet."))}</p>`;return}v.innerHTML='<div class="space-y-2">'+o.map(s=>{const u=s.title_zh||s.worksheet_title_zh||s.worksheet_slug,a=s.due_at?" · "+e("截止 ","Due ")+String(s.due_at).replace("T"," ").slice(0,16):"";return`<button type="button" class="assign-row w-full text-left border rounded-lg px-3 py-2 hover:border-indigo-300" data-id="${Number(s.id)}">
                    ...
                    </button>`}).join("")+"</div>",v.querySelectorAll(".assign-row").forEach(s=>{s.addEventListener("click",()=>q(parseInt(s.getAttribute("data-id")||"0",10)))})}async function q(l){if(!l)return;const o=await i.ScienceApi.apiFetch("/teacher/worksheet-assignments/"+l),s=o.assignment;z.textContent=(s.title_zh||s.worksheet_title_zh||e("習作","Assignment"))+" — "+e("評分","Grading");const u=o.submissions||[];w.innerHTML=`<table class="min-w-full text-sm"><thead class="bg-slate-50"><tr>
