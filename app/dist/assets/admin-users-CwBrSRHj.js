const a=window;function t(i,l){return a.AppRouter&&a.AppRouter.t?a.AppRouter.t(i,l):i}function e(i){return a.AppRouter&&a.AppRouter.escapeHtml?a.AppRouter.escapeHtml(i):String(i||"")}function E(i){return a.AppRouter&&a.AppRouter.spaHref?a.AppRouter.spaHref(i):String(i||"")}function N(){const i=document.getElementById("sidebar");i&&(i.style.display="none")}function L(i){return(a.ScienceApi&&a.ScienceApi.SITE_BASE||"")+"/admin/"+i}function k(){return a.ScienceApi.getUser()?!!a.ScienceApi.hasPermission("user.manage"):(a.AppRouter.navigate("/login"),!1)}async function _(){var m;N();const i=document.getElementById("page-title"),l=document.getElementById("card-container");if(i&&(i.textContent=t("使用者","Users")),!k()){a.ScienceApi.getUser()&&(l.innerHTML=`<p class="text-red-600">${e(t("沒有權限。","Forbidden."))}</p>`);return}l.innerHTML=`<p class="text-slate-500">${e(t("載入中…","Loading…"))}</p>`;try{let p=function(s,n){g&&(g.textContent=s,g.classList.remove("hidden","text-emerald-700","text-red-600"),g.classList.add(n?"text-red-600":"text-emerald-700"))};var A=p;const b=await a.ScienceApi.apiFetch("/admin/users"),v=b.users||[],y=b.roles||[],$=!!b.can_impersonate,x=a.ScienceApi.getUser(),f=x&&x.id?Number(x.id):0,o=y.map(s=>`<label class="inline-flex items-center gap-1.5 text-xs mr-3 mb-1">
                    <input type="checkbox" name="roles" value="${Number(s.id)}" class="rounded border-slate-300">
                    ${e(s.label||s.slug)}
                </label>`).join(""),w=v.map(s=>{if(s.is_system)return`<tr class="border-t border-slate-100 text-slate-400">
                    ...
                    <td class="p-3 whitespace-nowrap">
                        <a class="text-indigo-700 hover:underline" href="${e(E(`/admin/users/${Number(s.id)}`))}" data-spa-nav="/admin/users/${Number(s.id)}">${e(t("編輯","Edit"))}</a>
                        ${d}
                ...
                <div class="mb-4 flex flex-wrap gap-3 items-center">
                    <a href="${e(E("/admin"))}" data-spa-nav="/admin" class="text-sm text-indigo-700 hover:underline">${e(t("← 管理首頁","← Admin home"))}</a>
                    <a href="${e(L("permissions.php"))}" class="text-sm text-slate-600 hover:underline">${e(t("角色權限","Permissions"))}</a>
                    <a href="${e(L("users.php"))}" class="text-sm text-slate-600 hover:underline">${e(t("完整 PHP 列表","Full PHP list"))}</a>
                </div>
                ...
                <div class="mb-4 flex flex-wrap gap-3 items-center">
                    <a href="${e(E("/admin/users"))}" data-spa-nav="/admin/users" class="text-sm text-indigo-700 hover:underline">${e(t("← 使用者列表","← Users"))}</a>
                </div>
