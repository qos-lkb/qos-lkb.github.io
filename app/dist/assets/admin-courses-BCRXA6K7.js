const l=window;function t(c,a){return l.AppRouter&&l.AppRouter.t?l.AppRouter.t(c,a):c}function e(c){return l.AppRouter&&l.AppRouter.escapeHtml?l.AppRouter.escapeHtml(c):String(c||"")}function b(c){return l.AppRouter&&l.AppRouter.spaHref?l.AppRouter.spaHref(c):String(c||"")}function R(){const c=document.getElementById("sidebar");c&&(c.style.display="none")}function L(c,a){const v=Object.entries(c||{});return'<option value="">'+e(t("請選擇","Select"))+"</option>"+v.map(([h,k])=>`<option value="${e(h)}"${String(h)===String(a||"")?" selected":""}>${e(k)}</option>`).join("")}function B(c){const a=String(c||"").toUpperCase();return a==="E"?t("英文 (E)","English (E)"):a==="C"?t("中文 (C)","Chinese (C)"):"—"}function H(){if(!l.ScienceApi.getUser())return l.AppRouter.navigate("/login"),!1;const c=l.ScienceApi.hasPermission("class.manage_any"),a=l.ScienceApi.hasPermission("class.manage_own");return c||a}async function I(){var y,_,p;R();const c=document.getElementById("page-title"),a=document.getElementById("card-container");if(c&&(c.textContent=t("課程管理","Courses")),!H()){l.ScienceApi.getUser()&&(a.innerHTML=`<p class="text-red-600">${e(t("沒有權限。","Forbidden."))}</p>`);return}a.innerHTML=`<p class="text-slate-500">${e(t("載入中…","Loading…"))}</p>`;try{let i=function(s,n){f&&(f.textContent=s,f.classList.remove("hidden","text-emerald-700","text-red-600"),f.classList.add(n?"text-red-600":"text-emerald-700"))},r=function(s,n){try{sessionStorage.setItem("admin-courses-flash",JSON.stringify({msg:s,err:!!n}))}catch{}},u=function(){return Array.from(a.querySelectorAll(".admin-course-cb:checked")).map(s=>parseInt(s.value,10)).filter(s=>s>0)},m=function(){const s=document.getElementById("admin-courses-select-all"),n=a.querySelectorAll(".admin-course-cb"),d=a.querySelectorAll(".admin-course-cb:checked");s&&(s.checked=n.length>0&&d.length===n.length,s.indeterminate=d.length>0&&d.length<n.length);const A=document.getElementById("admin-courses-bulk-delete");A&&(A.disabled=d.length===0)};var v=i,h=r,k=u,N=m;const o=await l.ScienceApi.apiFetch("/admin/classes"),g=o.classes||[],j=o.form_level_options||{},E=o.course_subject_options||{},S=o.teacher_options||[],w=l.ScienceApi.hasPermission("class.manage_any"),x=l.ScienceApi.getUser(),$=(()=>{const s=new Date().getFullYear();return s+"-"+(s+1)})(),C=w&&S.length?`<label class="text-sm">${e(t("任教老師","Teacher"))}
                ...
                </label>`:`<input type="hidden" name="teacher_user_id" value="${Number(x.id)}">`,D=g.map(s=>{const n=s.is_active?`<span class="text-emerald-700 text-xs">${e(t("啟用","Active"))}</span>`:`<span class="text-slate-400 text-xs">${e(t("停用","Inactive"))}</span>`,d=String(s.name||"");return`<tr class="border-t border-slate-100">
                    <td class="p-3 w-10">
                ...
                <form id="admin-course-create" class="mb-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-3 items-end bg-white border border-slate-200 rounded-xl p-4">
                    ...
                    <label class="text-sm">${e(t("年級","Form"))}
                        <select name="form_level" required class="mt-1 w-full rounded border border-slate-300 px-2 py-1.5 text-sm">${L(j)}</select>
                    </label>
                    <label class="text-sm">${e(t("科目","Subject"))}
                        <select name="course_subject" required class="mt-1 w-full rounded border border-slate-300 px-2 py-1.5 text-sm">${L(E)}</select>
                    </label>
                    ${C}
                ...
                <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
                    <p class="text-sm text-slate-600">${g.length} ${e(t("門課程","courses"))}</p>
                ...
                <div class="bg-white rounded-xl border border-slate-200 overflow-x-auto shadow-sm">
                    <table class="min-w-full text-sm">
                        <thead class="bg-slate-100 text-left">
                            <tr>
                                <th class="p-3 w-10">
                                    ${g.length?`<input type="checkbox" id="admin-courses-select-all" class="rounded border-slate-300" aria-label="${e(t("全選","Select all"))}">`:""}
                                </th>
                ...
                <div class="mb-4 flex flex-wrap gap-3 items-center">
                    ...
                    ${o.can_chase_previous_summer||o.form_level==="2"||o.form_level==="3"?`<a href="${e(b(`/admin/courses/${a}/summer?cohort=previous`))}" data-spa-nav="/admin/courses/${a}/summer?cohort=previous" class="text-sm text-amber-800 hover:underline">${e(t("上學年追收","Last-year chase"))}</a>`:""}
