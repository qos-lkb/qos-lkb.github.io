import{_ as f}from"./index-ZTaAEhvb.js";const d=window;function e(s,r){return d.AppRouter&&d.AppRouter.t?d.AppRouter.t(s,r):s}function t(s){return d.AppRouter&&d.AppRouter.escapeHtml?d.AppRouter.escapeHtml(s):String(s||"")}function v(s){return d.AppRouter&&d.AppRouter.spaHref?d.AppRouter.spaHref(s):String(s||"")}function h(){const s=document.getElementById("sidebar");s&&(s.style.display="none")}function y(s){s.querySelectorAll("[data-spa-nav]").forEach(r=>{r.addEventListener("click",i=>{i.preventDefault(),d.AppRouter.navigate(r.getAttribute("data-spa-nav"))})})}function w(){const s=d.ScienceApi;return s.hasPermission("summer_homework.manage_any")||s.hasPermission("summer_homework.manage_own")}function $(s){return{mcq:e("選擇","MCQ"),multi_select:e("多選","Multi"),fill_blank:e("填充","Fill"),true_false:e("是非","T/F"),short_answer:e("短答","Short"),long_answer:e("長答","Long")}[s]||s}function x(s,r){if(d.AppAdminSummerQBuilder&&d.AppAdminSummerQBuilder.insertAtCursor){d.AppAdminSummerQBuilder.insertAtCursor(s,r);return}const i=s.selectionStart??s.value.length,o=s.selectionEnd??i,n=s.value;s.value=n.slice(0,i)+r+n.slice(o),s.selectionStart=s.selectionEnd=i+r.length,s.focus()}async function _(s,r){const i=new FormData;return i.append("file",r),d.ScienceApi.apiFetch("/admin/summer-homework/"+s+"/media",{method:"POST",body:i})}function k(s){s.querySelectorAll("[data-md-toolbar]").forEach(i=>{const o=i.getAttribute("data-md-toolbar"),n=document.getElementById(o);n&&i.querySelectorAll("[data-md-action]").forEach(a=>{a.addEventListener("click",async()=>{var l;const m=a.getAttribute("data-md-action");if(m==="bold")x(n,"**粗體**");else if(m==="ul")x(n,`
- 項目一
- 項目二
`);else if(m==="math")x(n,"$E=mc^2$");else if(m==="video")x(n,`
::video slug="your-video-slug"
`);else if(m==="article")x(n,`
::article slug="your-article-slug"
`);else if(m==="note")x(n,`
::note slug="your-note-slug"
`);else if(m==="image"){const c=parseInt(((l=document.getElementById("item-id"))==null?void 0:l.value)||"0",10);if(!c){alert(e("請先儲存習作後再上載圖片。","Save the item first, then upload images."));return}const u=document.createElement("input");u.type="file",u.accept="image/jpeg,image/png,image/gif,image/webp",u.onchange=async()=>{const p=u.files&&u.files[0];if(p)try{const b=await _(c,p);x(n,(b.markdown||`![${p.name}](${b.url||""})`)+`
`)}catch(b){alert(b.message||e("上載失敗","Upload failed"))}},u.click()}})})}),s.querySelectorAll("[data-md-preview]").forEach(i=>{i.addEventListener("click",async()=>{const o=i.getAttribute("data-md-preview"),n=document.getElementById(o),a=document.getElementById(o+"-preview");if(!n||!a)return;a.classList.remove("hidden");const m=n.value||"";let l=d.AppMarkdown&&d.AppMarkdown.renderMarkdownToHtml?d.AppMarkdown.renderMarkdownToHtml(m):t(m).replace(/\n/g,"<br>");a.innerHTML=l,d.AppContentEmbeds&&d.AppContentEmbeds.hydrate&&await d.AppContentEmbeds.hydrate(a),d.MathJax&&d.MathJax.typesetPromise&&d.MathJax.typesetPromise([a]).catch(()=>{})})})}function E(s){var m;const r=document.getElementById("content-refs-list"),i=document.getElementById("content-refs-json");if(!r||!i)return;function o(){try{const l=i.value.trim(),c=l?JSON.parse(l):[];return Array.isArray(c)?c:[]}catch{return[]}}function n(l){i.value=JSON.stringify(l,null,2),a()}function a(){const l=o();if(!l.length){r.innerHTML=`<p class="text-xs text-slate-500">${t(e("尚未引用平台內容。","No content references yet."))}</p>`;return}r.innerHTML=l.map((c,u)=>`<div class="flex items-center gap-2 text-sm border rounded-lg px-3 py-2 bg-slate-50">
            ...
            </div>`).join(""),r.querySelectorAll("[data-ref-rm]").forEach(c=>{c.addEventListener("click",()=>{const u=o();u.splice(parseInt(c.getAttribute("data-ref-rm"),10),1),n(u)})})}(m=document.getElementById("content-ref-add"))==null||m.addEventListener("click",()=>{var b,g;const l=((b=document.getElementById("content-ref-type"))==null?void 0:b.value)||"note",c=(((g=document.getElementById("content-ref-slug"))==null?void 0:g.value)||"").trim();if(!c){alert(e("請輸入 slug","Enter a slug"));return}const u=o();u.push({type:l,slug:c}),n(u);const p=document.getElementById("content-ref-slug");p&&(p.value="")}),a()}function I(){var i,o;const s=document.getElementById("sh-import-qb"),r=document.getElementById("sh-import-dialog");!s||!r||(s.addEventListener("click",()=>{var a;if(!parseInt(((a=document.getElementById("item-id"))==null?void 0:a.value)||"0",10)){alert(e("請先儲存習作後再匯入題目。","Save the item first, then import questions."));return}r.classList.remove("hidden")}),(i=document.getElementById("sh-import-cancel"))==null||i.addEventListener("click",()=>{r.classList.add("hidden")}),(o=document.getElementById("sh-import-run"))==null||o.addEventListener("click",async()=>{var u,p,b;const n=parseInt(((u=document.getElementById("item-id"))==null?void 0:u.value)||"0",10),a=parseInt(((p=document.getElementById("sh-import-bank"))==null?void 0:p.value)||"0",10),l=(((b=document.getElementById("sh-import-qids"))==null?void 0:b.value)||"").trim().split(/[\s,]+/).map(g=>parseInt(g,10)).filter(g=>g>0);if(!n||!a||!l.length){alert(e("請填寫試題庫 ID 與題目 ID。","Provide bank id and question ids."));return}const c=document.getElementById("edit-flash");try{const g=await d.ScienceApi.apiFetch("/admin/summer-homework/"+n+"/import-questions",{method:"POST",body:{bank_id:a,question_ids:l}});r.classList.add("hidden"),c&&(c.textContent=e(`已匯入 ${g.imported||l.length} 題。`,`Imported ${g.imported||l.length} questions.`),c.classList.remove("hidden","text-red-600"),c.classList.add("text-emerald-700")),d.AppRouter.navigate("/admin/summer-homework/"+n+"/edit")}catch(g){alert(g.message||e("匯入失敗","Import failed"))}}))}function A(s){return{draft:e("草稿","Draft"),pending_review:e("待審核","Pending"),published:e("已發佈","Published")}[s]||s||"—"}function S(s,r){const i=s.stem_zh||s.stem_en||"",o=s.question_type||"";let n="";if(o==="mcq"||o==="multi_select")n=`<ul class="text-sm space-y-1.5">${(s.options||[]).map((a,m)=>{const l=String.fromCharCode(65+m),c=a.text_zh||a.text_en||"",u=a.is_correct===!0||a.is_correct===1||a.is_correct==="1";return`<li class="${u?"text-emerald-800 font-medium":"text-slate-700"}">
                    <span class="font-bold text-indigo-600 mr-1">${l}</span>${t(c)}
                    ${u?`<span class="text-xs text-emerald-700 ml-1">✓ ${t(e("正確答案","Correct"))}</span>`:""}
                </li>`}).join("")}</ul>`;else if(o==="true_false"){const a=s.correct_bool===!0||s.correct_bool===1||s.correct_bool==="1";n=`<p class="text-sm text-emerald-800 font-medium">${t(e("正確答案：","Answer: "))}${t(a?e("是","True"):e("否","False"))}</p>`}else o==="short_answer"?n=`<ul class="text-sm space-y-1">${(s.acceptable_answers||[]).map(a=>!a||typeof a!="object"?"":`<li class="font-mono text-emerald-800">${t(a.acceptable_answer_zh||"")} / ${t(a.acceptable_answer_en||"")}</li>`).join("")}</ul>`:o==="long_answer"?(n=`<p class="text-sm text-slate-600">${t(e("滿分","Max"))} ${t(String(s.max_score??5))}（${t(e("教師評閱","Teacher-marked"))}）</p>`,(s.rubric_zh||s.rubric_en)&&(n+=`<div class="mt-2 text-xs text-slate-600 space-y-1">
                ...
                </div>`)):n=(s.blanks||[]).map((a,m)=>{const l=a.acceptable_answers||[];return`<div class="text-sm mb-2">
                    <span class="text-slate-500">${t(e("空格","Blank"))} ${m+1}：</span>
                    ${l.map(c=>`<span class="font-mono text-emerald-800 mr-2">${t((c.acceptable_answer_zh||"")+" / "+(c.acceptable_answer_en||""))}</span>`).join("")}
            ...
            <p class="font-medium text-slate-900 mb-3">
                ${r+1}.
                <span class="text-xs font-normal px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 mr-1">${t($(o))}</span>
                ${t(i)}
            </p>
            ${n}
        </div>`}async function L(s){h();const r=document.getElementById("page-title"),i=document.getElementById("card-container"),o=s?Number(s):0;if(r&&(r.textContent=o?e("編輯暑期功課","Edit summer homework"):e("新增暑期功課","New summer homework")),!d.ScienceApi.getUser()){d.AppRouter.navigate("/login");return}if(!w()){i.innerHTML=`<p class="text-red-600">${t(e("沒有權限。","Forbidden."))}</p>`;return}if(o)try{if((await d.ScienceApi.apiFetch("/admin/summer-homework/"+o)).can_manage===!1){d.AppRouter.navigate("/admin/summer-homework/"+o+"/view");return}}catch(m){i.innerHTML=`<p class="text-red-600">${t(m.message||e("載入失敗","Load failed"))}</p>`;return}const n=Number(new URLSearchParams(location.search).get("regraded")||0);i.innerHTML=`
            ...
            <div id="sh-import-dialog" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
                ...
                <div class="mb-4 flex flex-wrap gap-3 items-center text-sm">
                    ...
                    ${n.can_manage?`<a href="${t(v("/admin/summer-homework/"+o+"/edit"))}" data-spa-nav="/admin/summer-homework/${o}/edit" class="text-indigo-700 font-medium hover:underline">${t(e("編輯","Edit"))}</a>`:""}
                </div>
                <p class="text-xs text-slate-500 mb-4">${t(a)} · ${t(m)} · ${t(A(n.status))} · ${t(e("含正確答案（教師／管理員檢視）","Includes answer key (staff view)"))}</p>
                ...
                <div class="space-y-4 mb-8">
                    ...
                    ${l.length?l.map((u,p)=>S(u,p)).join(""):`<p class="text-slate-500 text-sm">${t(e("尚無題目。","No questions yet."))}</p>`}
                </div>`,y(i)}catch(n){i.innerHTML=`<p class="text-red-600">${t(n.message||e("載入失敗","Load failed"))}</p>`}}async function R(s){h();const r=document.getElementById("page-title"),i=document.getElementById("card-container"),o=Number(s||0);if(!o){d.AppRouter.navigate("/admin/summer-homework");return}if(!d.ScienceApi.getUser()){d.AppRouter.navigate("/login");return}r&&(r.textContent=e("預覽暑期功課","Preview summer homework")),i.innerHTML=`<p class="text-slate-500">${t(e("載入中…","Loading…"))}</p>`;try{await Promise.all([f(()=>import("./markdown-D3_y0rxP.js"),[],import.meta.url),f(()=>import("./content-embeds-BF_jUCgb.js"),[],import.meta.url),f(()=>import("./summer-homework-216J18Ad.js"),[],import.meta.url)]);const n=await d.ScienceApi.apiFetch("/admin/summer-homework/"+o),a=n.slug||String(o),m=Object.assign({},n,{include_answers:!1,can_review:!1,progress:null,submissions_closed:!1}),l=document.createElement("div");l.id="sh-admin-preview-root",i.innerHTML="",i.appendChild(l),await d.AppSummerHomework.renderItem(a,{root:l,item:m,preview:!0,forceLang:d.AppRouter.getLang?d.AppRouter.getLang():"zh",onBack:()=>d.AppRouter.navigate("/admin/summer-homework/"+o+"/edit")})}catch(n){i.innerHTML=`<p class="text-red-600">${t(n.message||e("載入失敗","Load failed"))}</p>
                <a href="${t(v("/admin/summer-homework"))}" data-spa-nav="/admin/summer-homework" class="text-indigo-700 text-sm mt-3 inline-block">${t(e("← 返回列表","← Back to list"))}</a>`,y(i)}}Object.assign(d.AppAdmin||(d.AppAdmin={}),{renderAdminSummerHomeworkEdit:L,renderAdminSummerHomeworkView:B,renderAdminSummerHomeworkPreview:R});
