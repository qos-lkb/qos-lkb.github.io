const o=window;function t(r,u){return o.AppRouter&&o.AppRouter.t?o.AppRouter.t(r,u):r}function i(r){return o.AppRouter&&o.AppRouter.escapeHtml?o.AppRouter.escapeHtml(r):String(r||"")}function T(r){return o.AppRouter&&o.AppRouter.spaHref?o.AppRouter.spaHref(r):String(r||"")}function C(){const r=document.getElementById("sidebar");r&&(r.style.display="none")}const $={note:()=>t("學習筆記","Learning note"),simulation:()=>t("模擬實驗","Simulation"),worksheet:()=>t("工作紙","Worksheet"),article:()=>t("科學文章","Article"),learning_tool:()=>t("互動測驗","Quiz"),question_bank:()=>t("試題庫","Question bank"),flashcard_set:()=>t("閃卡","Flashcards"),video:()=>t("影片","Video")};function N(){const r=o.ScienceApi;return!r||!r.getUser()?!1:r.hasPermission("topic_item.manage_any")||r.hasPermission("user.manage")}function F(r,u,f,E){if(!r)return;let s=null;function v(a){return Array.prototype.slice.call(r.querySelectorAll(u+":not(.dragging)")).reduce((m,d)=>{const y=d.getBoundingClientRect(),h=a-y.top-y.height/2;return h<0&&h>m.offset?{offset:h,element:d}:m},{offset:Number.NEGATIVE_INFINITY,element:void 0}).element}r.addEventListener("dragenter",a=>a.preventDefault()),r.addEventListener("dragover",a=>{if(a.preventDefault(),a.dataTransfer&&(a.dataTransfer.dropEffect="move"),!s)return;const p=v(a.clientY);p==null?r.appendChild(s):r.insertBefore(s,p)}),r.addEventListener("drop",a=>a.preventDefault()),r.querySelectorAll(u).forEach(a=>{const p=a.querySelector(f);if(!p)return;const m=()=>{a.setAttribute("draggable","true"),a.dataset.sortArmed="1"};p.addEventListener("pointerdown",m),p.addEventListener("mousedown",m),a.addEventListener("dragend",()=>{a.removeAttribute("draggable"),delete a.dataset.sortArmed,a.classList.remove("dragging","opacity-60"),s===a&&(s=null,E())}),a.addEventListener("dragstart",d=>{if(a.dataset.sortArmed!=="1"){d.preventDefault();return}delete a.dataset.sortArmed,s=a,a.classList.add("dragging","opacity-60"),d.dataTransfer.effectAllowed="move";try{d.dataTransfer.setData("text/plain","sort")}catch{}})})}async function M(){C();const r=document.getElementById("page-title"),u=document.getElementById("card-container");if(r&&(r.textContent=t("自學課程編排","Course curriculum")),!o.ScienceApi.getUser()){o.AppRouter.navigate("/login");return}if(!N()){u.innerHTML=`<p class="text-red-600">${i(t("沒有權限。","Forbidden."))}</p>`;return}u.innerHTML=`<p class="text-slate-500">${i(t("載入中…","Loading…"))}</p>`;let f=[];try{f=await o.ScienceApi.apiFetch("/admin/subjects"),Array.isArray(f)||(f=[])}catch(e){u.innerHTML=`<p class="text-red-600">${i(e.message||t("載入失敗","Load failed"))}</p>`;return}const E=Object.keys($).map(e=>`<option value="${i(e)}">${i($[e]())}</option>`).join("");u.innerHTML=`
            <style>
